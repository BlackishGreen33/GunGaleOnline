    ...
    def update(self, debug=False):
        if (
            ...
            for wall in self.map.inside_walls:
                nearest_point = list(
                    ops.nearest_points(
                        geometry.Point(self.player.center), wall.shapely
                    )[1].coords
                )[0]
                wall.distance = round(
                    (
                        (nearest_point[0] - self.player.center[0]) ** 2
                        + (nearest_point[1] - self.player.center[1]) ** 2
                    )
                    ** 0.5,
                    2,
                )
            ...
            for wall in sorted(self.map.inside_walls, key=lambda x: x.distance):
                ...
                for corner in wall.corners:
                    ...
                    evx = vx / (vx**2 + vy**2) ** 0.5
                    evy = vy / (vx**2 + vy**2) ** 0.5
                ...
                shadow_indices = spatial.qhull.ConvexHull(allpoints).vertices
