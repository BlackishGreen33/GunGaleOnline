...
class MainScene:
    ...
    def handle_input(self, input):
        ...
        if (keys[pygame.K_a] and keys[pygame.K_d]) or (
            not keys[pygame.K_a] and not keys[pygame.K_d]
        ):
            self.player.stop_x()
        if (keys[pygame.K_w] and keys[pygame.K_s]) or (
            not keys[pygame.K_w] and not keys[pygame.K_s]
        ):
            self.player.stop_y()
...
class HostScene(MainScene):
        ...
        info = {
            "players": [
                [
                    ...
                    self.player.active_weapon,
                    self.player.frame,
                    self.player.get_new_bullets(),
                    self.player.team,
                    self.player.hearts,
                ]
            ]
    ...
    def handle_client(self, client):
        ...
        info = {
            ...
            + [
                [p.center, p.rotation, p.active_weapon, p.frame, p.team, p.hearts]
                for p in self.player_list
            ],
        }
        ...
        client_session_info = json.loads(
            client_session_info.split(self.message_splitter)[0]
        )
            ...
            try:
                ...
                if msg != bytes("{quit}", "utf8"):
                    ...
                    if "player" in client_info:
                        ...
                        new_player.set_image(
                            client_info["player"]["weapon"],
                            client_info["player"]["frame"],
                        )
...
class ClientScene(MainScene):
    def __init__(self, server_info=("127.0.0.1", 33000), client_info=None):
        ...
        if client_info is None:
            self.client_info = {"name": "name", "team": "team"}
        else:
    ...
    def update(self, surface, input):
        ...
        if len(self.player.damage_taken) > 0:
            self.send(
                self.build_message(
                    {"damage": self.player.get_damage_taken(self.own_index)}
                )
            )
...
class MainMenuScene(MenuScene):
    def __init__(self):
        ...
        self.menu_content = [
            ...
            menu.Button(
                "settings", self.settings_image.get_rect(), image=self.settings_image
            ),
        ]
        ...
        if self.menu.get_pressed("join"):
            if self.join_clicked == 1:
                ...
                if self.host_clicked == 0:
                    self.menu.add_content(
                        menu.Input(
                            "ip", self.input_image.get_rect(), image=self.input_image
                        ),
                        1,
                    )
                    self.menu.add_content(
                        menu.Input(
                            "port", self.input_image.get_rect(), image=self.input_image
                        ),
                        2,
                    )
                else:
            ...
            if self.host_clicked == 1:
                ...
                self.menu.add_content(
                    menu.Input(
                        "host port", self.input_image.get_rect(), image=self.input_image
                    ),
                    0,
                )
        ...
        try:
            ...
            if not self.host_port_error:
                self.menu.add_content(
                    menu.Text(
                        "host port warning",
                        "seems like the port is not a number",
                        (169, 59, 59),
                    )
                )
            ...
            if not self.host_connect_error:
                self.menu.add_content(
                    menu.Text(
                        "host connect warning",
                        "seems like the port is already taken",
                        (169, 59, 59),
                    )
                )
    ...
    def test_join(self):
        try:
            test_address = (
                self.menu.get_text("ip").strip(),
                int(self.menu.get_text("port").strip()),
            )
            ...
            if not self.join_connect_error:
                self.menu.add_content(
                    menu.Text(
                        "join connect warning",
                        "no such game session found",
                        (169, 59, 59),
                    )
                )
...
class CreateHostScene(MenuScene):
    def __init__(self, port):
        ...
        self.menu_content = [
            menu.Input(
                "your player name", self.input_image.get_rect(), image=self.input_image
            ),
            menu.Input(
                "team name 1, team name 2",
                self.input_image.get_rect(),
    ...
    def handle_menu_actions(self):
        if self.menu.get_pressed("host"):
            ...
            teams = [
                t.strip()
                for t in self.menu.get_text("team name 1, team name 2").split(",")
            ]
            ...
            if name == "":
                if not self.name_empty:
                    self.menu.add_content(
                        menu.Text(
                            "name empty", "please enter you player name", (169, 59, 59)
                        )
                    )
            ...
            if len(name) > 15:
                if not self.name_long:
                    self.menu.add_content(
                        menu.Text(
                            "name long",
                            "your player name is to long (max: 15)",
                            (169, 59, 59),
                        )
                    )
                    self.name_long = True
            ...
            if len(teams) < 2:
                if not self.teams_less:
                    self.menu.add_content(
                        menu.Text(
                            "teams less", "please enter a least 2 teams", (169, 59, 59)
                        )
                    )
            ...
            if len(teams) > 4:
                if not self.teams_many:
                    self.menu.add_content(
                        menu.Text(
                            "teams many",
                            "please don´t enter more than 4 teams",
                            (169, 59, 59),
                        )
                    )
            ...
            if len(teams) != len(set(teams)):
                if not self.team_same:
                    self.menu.add_content(
                        menu.Text(
                            "teams same",
                            "the teams should not have same names",
                            (169, 59, 59),
                        )
                    )
                    self.team_same = True
            ...
            if "" in teams:
                if not self.team_empty:
                    self.menu.add_content(
                        menu.Text(
                            "team empty",
                            "empty team names are not allowed",
                            (169, 59, 59),
                        )
                    )
            ...
            if own_team not in teams:
                if not self.team_not_in_teams:
                    self.menu.add_content(
                        menu.Text(
                            "team not in teams",
                            "the team you try to join does not exist",
                            (169, 59, 59),
                        )
                    )
...
class CreateJoinScene(MenuScene):
    def __init__(self, address, session_info):
        ...
        self.menu = menu.Menu(
            "center", (100, 50), f"join {host_name}´s game", self.menu_content
        )
        ...
        if self.menu.get_pressed("join"):
            ...
            if team == "":
                if not self.team_empty:
                    self.menu.add_content(
                        menu.Text(
                            "team empty", "please enter you player team", (169, 59, 59)
                        )
                    )
                    self.team_empty = True
            elif self.team_empty:
            ...
            if not any(
                ...
                client_info = {"name": name, "team": team}
