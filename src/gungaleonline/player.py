...
class Player:
    ...
    def update(self, enemies=None):
        ...
        self.center = (
            self.center[0] + self.dx * self.dt,
            self.center[1] + self.dy * self.dt,
        )
        ...
        self.check_collision_x(
            (self.center[0], old_rect.center[1]),
            pygame.Rect(self.rect.x, old_rect.y, self.rect.width, old_rect.height),
        )
    ...
    def attack(self, clicked=False):
        if (
            self.can_attack
            and (self.ammo[1] != 0 or self.active_weapon == "knife")
            ...
            if self.active_weapon == "rifle" or self.active_weapon == "pistol":
                ...
                direction_speed = (direction[0] ** 2 + direction[1] ** 2) ** 0.5
                direction = (
                ...
                center = (
                    self.center[0] + direction[0] * 30,
                    self.center[1] + direction[1] * 30,
                )
                ...
                if self.active_weapon == "pistol":
                    ...
                    self.new_bullets.append(
                        [direction, center, self.bullets_speed, self.pistol_damage]
                    )
                else:
                    self.bullets.append(
                        bullet.Bullet(
                            direction,
                            center,
                            self.bullets_speed / 2,
                            self.rifle_damage,
                            self.map,
                        )
                    )
                    self.new_bullets.append(
                        [direction, center, self.bullets_speed / 2, self.rifle_damage]
                    )
    ...
    def rotate(self):
        mouse_pos = to_renderer_position(pygame.mouse.get_pos())
        radians = math.atan2(
            mouse_pos[1] - self.center[1], mouse_pos[0] - self.center[0]
        )
